...
export const timingSchema = z.object({
    ...
    breakStartTime: z.string().regex(timeRegex, "Invalid time format (HH:MM)"),
    breakStartTimeAmPm: z.enum(['AM', 'PM'] as [AmPm, ...AmPm[]]),
    breakEndTime: z.string().regex(timeRegex, "Invalid time format (HH:MM)"),
    breakEndTimeAmPm: z.enum(['AM', 'PM'] as [AmPm, ...AmPm[]]),
}).refine((data) => {
    ...
    const shiftStart = getMinutes(data.startTime, data.startTimeAmPm);
    let shiftEnd = getMinutes(data.endTime, data.endTimeAmPm);
    if (shiftEnd < shiftStart) shiftEnd += 24 * 60;

    let breakStart = getMinutes(data.breakStartTime, data.breakStartTimeAmPm);
    let breakEnd = getMinutes(data.breakEndTime, data.breakEndTimeAmPm);

    // Normalize break times relative to shift start
